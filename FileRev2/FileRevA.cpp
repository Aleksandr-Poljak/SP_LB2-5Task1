					          ...
					          LPSTR lpszCmdLine, 	int nCmdShow) 
{
	...
	LPSTR lpszCmdLineT; // Указатель на командную строку 
                         // в ANSI или UNICODE
  ...
  // - символы, увеличиваем файл на величину размера символа WCHAR
	...
	hFileMap = CreateFileMapping(	
                    ...
                    PAGE_READWRITE, // Атрибуты защиты страниц 
		             0, //и старшие 32 разряда размера файла.
                    dwFileSize+sizeof(WCHAR),   //Младшие 32 разряда
                    NULL  // имя объекта ядра 
                    );
	if (hFileMap == NULL) 
  ...
  // чтобы увидеть плоды своих трудов
	...
	si.dwFlags = STARTF_USESHOWWINDOW;// Устанавливаем флаг - учитывать значение поля wShowWindow
	char CmdParam[] = "NOTEPAD.EXE " FILENAME;
